...
# from `enum git_cert_t` in `cert.h`
...
function certificate_callback(
    ...
    transport = cert_type == CERT_TYPE_TLS ? "TLS" :
                cert_type == CERT_TYPE_SSH ? "SSH" : nothing
    @show host, transport
    return NetworkOptions.verify_host(host, transport)
end
